...
# TODO: вынести путь бэкенд-весов в параметры безового класса
...
class FullYoloFeatureScratch(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
...
class TinyYoloFeatureScratch(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
...
class ModB_YoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
        ...
        # Layer 5
        x = Conv2D(32, (1, 1), strides=(1, 1), padding='same', name='conv_5', use_bias=False)(x)
        x = BatchNormalization(name='norm_5')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 6
        x = Conv2D(32, (1, 1), strides=(1, 1), padding='same', name='conv_6', use_bias=False)(x)
        x = BatchNormalization(name='norm_6')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 7
        x = Conv2D(64, (3, 3), strides=(1, 1), padding='same', name='conv_7', use_bias=False)(x)
        x = BatchNormalization(name='norm_7')(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        ...
        # Layer 10
        x = Conv2D(32, (1, 1), strides=(1, 1), padding='same', name='conv_10', use_bias=False)(x)
        x = BatchNormalization(name='norm_10')(x)
        x = LeakyReLU(alpha=0.1)(x)
        ...
        # Layer 14
        x = Conv2D(128, (1, 1), strides=(1, 1), padding='same', name='conv_14', use_bias=False)(x)
        x = BatchNormalization(name='norm_14')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 15
        x = Conv2D(256, (3, 3), strides=(1, 1), padding='same', name='conv_15', use_bias=False)(x)
        x = BatchNormalization(name='norm_15')(x)
        x = LeakyReLU(alpha=0.1)(x)
        ...
        # Layer 18
        skip_connection = Conv2D(256, (3, 3), strides=(1, 1), padding='same', name='conv_18', use_bias=False)(
            skip_connection)
        skip_connection = BatchNormalization(name='norm_18')(skip_connection)
        skip_connection = LeakyReLU(alpha=0.1)(skip_connection)
        #skip_connection = Lambda(space_to_depth_x2)(skip_connection)

        # Layer 19
        skip_connection = Conv2D(64, (1, 1), strides=(1, 1), padding='same', name='conv_19', use_bias=False)(
            skip_connection)
        skip_connection = BatchNormalization(name='norm_19')(skip_connection)
        skip_connection = LeakyReLU(alpha=0.1)(skip_connection)
        skip_connection = Lambda(space_to_depth_x2)(skip_connection)
...
class Mod3_YoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
        ...
        # Layer 13
        skip_connection = Conv2D(256, (3, 3), strides=(1, 1), padding='same', name='conv_13', use_bias=False)(
            skip_connection)
        skip_connection = BatchNormalization(name='norm_13')(skip_connection)
        skip_connection = LeakyReLU(alpha=0.1)(skip_connection)
        #skip_connection = Lambda(space_to_depth_x2)(skip_connection)

        # Layer 14
        skip_connection = Conv2D(64, (1, 1), strides=(1, 1), padding='same', name='conv_14', use_bias=False)(
            skip_connection)
        skip_connection = BatchNormalization(name='norm_14')(skip_connection)
        skip_connection = LeakyReLU(alpha=0.1)(skip_connection)
        skip_connection = Lambda(space_to_depth_x2)(skip_connection)
        ...
        # Layer15
        x = Conv2D(512, (3, 3), strides=(1, 1), padding='same', name='conv_15', use_bias=False)(x)
        x = BatchNormalization(name='norm_15')(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class Mod4_YoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
        ...
        # Layer 5
        x = Conv2D(64, (1, 1), strides=(1, 1), padding='same', name='conv_5', use_bias=False)(x)
        x = BatchNormalization(name='norm_5')(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class Mod5_YoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
        ...
        # Layer 13
        skip_connection = MaxPooling2D(pool_size=(2, 2))(skip_connection) # не уверена насчет этого
        ...
        # Layer16
        x = Conv2D(256, (1, 1), strides=(1, 1), padding='same', name='conv_16', use_bias=False)(x)
        x = BatchNormalization(name='norm_16')(x)
        x = LeakyReLU(alpha=0.1)(x)
        ...
        # Layer18
        x = Conv2D(256, (1, 1), strides=(1, 1), padding='same', name='conv_18', use_bias=False)(x)
        x = BatchNormalization(name='norm_18')(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class Mod6_YoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        input_image = Input(shape=(input_size[1], input_size[0], 3))
        ...
        # Layer 7
        skip_connection = Conv2D(256, (3, 3), strides=(1, 1), padding='same', name='conv_7', use_bias=False)(
            skip_connection)
        skip_connection = BatchNormalization(name='norm_7')(skip_connection)
        skip_connection = LeakyReLU(alpha=0.1)(skip_connection)

        # Layer 8
        skip_connection = Conv2D(64, (1, 1), strides=(1, 1), padding='same', name='conv_8', use_bias=False)(
            skip_connection)
        skip_connection = BatchNormalization(name='norm_8')(skip_connection)
        skip_connection = LeakyReLU(alpha=0.1)(skip_connection)
        skip_connection = Lambda(space_to_depth_x2)(skip_connection)
...
class VGG16Feature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        vgg16 = VGG16(input_shape=(input_size[1], input_size[0], 3), include_top=False)
...
class ResNet50Feature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size):
        resnet50 = ResNet50(input_shape=(input_size[1], input_size[0], 3), include_top=False)
