...
from backend import TinyYoloFeature, TinyYoloFeatureScratch, FullYoloFeature, FullYoloFeatureScratch, ModB_YoloFeature, \
    Mod3_YoloFeature, Mod4_YoloFeature, Mod5_YoloFeature, Mod6_YoloFeature, MobileNetFeature, SqueezeNetFeature, \
...
class YOLO(object):
    def __init__(self, architecture,
        ...
        elif architecture == 'Full Yolo Scratch':
            self.feature_extractor = FullYoloFeatureScratch(self.input_size)
        elif architecture == 'Tiny Yolo':
            self.feature_extractor = TinyYoloFeature(self.input_size)
        elif architecture == 'Tiny Yolo Scratch':
            self.feature_extractor = TinyYoloFeatureScratch(self.input_size)
        elif architecture == 'Mod B Yolo':
            self.feature_extractor = ModB_YoloFeature(self.input_size)
        elif architecture == 'Mod 3 Yolo':
            self.feature_extractor = Mod3_YoloFeature(self.input_size)
        elif architecture == 'Mod 4 Yolo':
            self.feature_extractor = Mod4_YoloFeature(self.input_size)
        elif architecture == 'Mod 5 Yolo':
            self.feature_extractor = Mod5_YoloFeature(self.input_size)
        elif architecture == 'Mod 6 Yolo':
            self.feature_extractor = Mod6_YoloFeature(self.input_size)
    ...
    def predict(self, image, threshold = 0.5):
        ...
        image = self.feature_extractor.normalize(image) # это легко
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        self.model.compile(loss=self.custom_loss, metrics=['accuracy', self.custom_loss], optimizer=optimizer)
        ...
        checkpoint_best = ModelCheckpoint(logdir + saved_weights_name + '.hdf5',
                                          monitor='val_loss',
                                          verbose=1,
                                          save_best_only=True,
        ...
        my_callbacks = [checkpoint_best, tensorboard, reduce_lr, time_callback, csv_logger]
        if saving_freq > 0:
            checkpoint_period = ModelCheckpoint(logdir + 'weights{epoch:03d}.hdf5',
                                                monitor='val_loss',
                                                verbose=1,
...
class TimeCallback(Callback):
    ...
    def on_train_begin(self, logs=None):
        self.epoch = 0
        self.time_full = timer()
        line = 'training start: ' + str(self.time_full) + '\n'
        with open(self.logdir + 'timelog.txt', 'w') as f:
            f.write(line)
    ...
    def on_batch_end(self, batch, logs=None):
        self.time_batch = timer() - self.time_batch
        self.time_full  = timer() - self.time_full
        line = '{}_{}:\tabs_time {}, rel_time {}\n'.format(self.epoch, batch, self.time_full, self.time_batch)
        with open(self.logdir + 'timelog.txt', 'a') as f:
            f.write(line)
